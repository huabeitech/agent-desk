...
export function CommunityCategoryNav(props: CommunityCategoryNavProps) {
  ...
  return (
    <aside className="hidden min-w-0 self-start overflow-hidden rounded-md bg-card xl:sticky xl:top-[5.5rem] xl:block" aria-label={t("supportPublic.posts.categoryNavigation")}>
      <nav>
        <CommunityCategoryMenuContent {...props} contentClassName="px-3 py-4" />
      </nav>
    </aside>
  )
}

export function CommunityCategoryMenuContent({
  categories,
  active,
  loading = false,
  failed = false,
  ...
  className,
  contentClassName,
}: CommunityCategoryNavProps & { className?: string; contentClassName?: string }) {
  const t = useI18n()
