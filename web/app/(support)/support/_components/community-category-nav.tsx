...
export function CommunityCategoryNav(props: CommunityCategoryNavProps) {
  const t = useI18n()
  return (
    <aside className="hidden min-w-0 self-start overflow-hidden rounded-md bg-card xl:sticky xl:top-[5.5rem] xl:block" aria-label={t("supportPublic.posts.categoryNavigation")}>
      <nav>
        <CommunityCategoryMenuContent {...props} className="px-3 py-4" />
      </nav>
    </aside>
  )
}

export function CommunityCategoryMenuContent({ categories, active, loading = false, failed = false, onChange, onRetry, onSelected, className }: CommunityCategoryNavProps & { className?: string }) {
  const t = useI18n()
  ...
  return (
    <div className={cn("grid max-h-[calc(100svh-7.625rem)] gap-0.5 overflow-y-auto", className)}>
