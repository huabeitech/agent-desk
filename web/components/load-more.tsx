...
export function LoadMore<T>({
  ...
  const contentKey = React.useMemo(
    ...
    [initialCursor, initialHasMore, initialLoad, resetKey, safeInitialItems.length]
  )
...
function LoadMoreContent<T>({
  ...
  const [hasMore, setHasMore] = React.useState(initialHasMore || (initialLoad && initialItems.length === 0))
  const [items, setItems] = React.useState<T[]>(initialItems)
  const [loaded, setLoaded] = React.useState(!initialLoad || initialItems.length > 0)
  ...
  const loadMore = React.useCallback(
    async (force = false) => {
      ...
      try {
        ...
        setItems((current) => (force ? data.results || [] : [...current, ...(data.results || [])]))
        setCursor(data.cursor || "")
        setHasMore(Boolean(data.hasMore))
        setLoaded(true)
      } catch (err) {
        if (mountedRef.current) {
          setError(err instanceof Error ? err.message : labels.error || "Could not load more items.")
        }
      } finally {
  ...
  return (
    <>
      {items.length ? renderItems(items) : loaded && !loading && !error ? renderEmpty?.() : null}
      {showButton ? (
        <LoadMoreButton
          loading={loading}
          hasMore={hasMore}
      ...
      {autoLoadOnScroll ? <div ref={sentinelRef} aria-hidden="true" className="h-px" /> : null}
      {error ? <p className="-mt-4 pb-5 text-center text-xs text-destructive">{error}</p> : null}
    </>
  )
}
...
export function LoadMoreButton({
  ...
  return (
    <div className="p-5 text-center">
      <Button type="button" variant="link" disabled={loading || !hasMore} onClick={onClick} className="w-[150px]">
        {loading ? loadingLabel || labels.loading || labels.loadMore : hasMore ? labels.loadMore : labels.noMore}
      </Button>
    </div>
  )
}
